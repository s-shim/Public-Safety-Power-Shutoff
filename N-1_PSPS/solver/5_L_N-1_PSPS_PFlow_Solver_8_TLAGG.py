...
model.setParam('LogFile', 'result%s/grblogL_AGG_TL%s_%s_Relax%s_N-1_PSPS_Span%s_PFlow_Lazy%s_%s_inst%s.txt'%(len(nodeArray),TL,machineName,relax,spanning,laziness,len(nodeArray),inst))   
...
#     resultTable.to_csv(r'result%s/resultL_%s_N-1_PSPS_Span%s_PFlow_inst%s_Lazy%s_%s.csv'%(len(nodeArray),machineName,spanning,inst,laziness,len(nodeArray)), index = False)#Check
# =============================================================================
