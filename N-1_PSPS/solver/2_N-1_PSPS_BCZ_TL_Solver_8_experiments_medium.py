...
for spanning in [False]:#[True, False]:
    for (numNodes,folderName,TL) in [(118,'matpower/case%s'%(118),60),(300,'matpower/case%s'%(300),900)]:
        print(machineName)
        ...
        for inst in range(1,50+1):
            ...
            model.setParam('LogFile', 'result%s/grblog_%s_N-1_PSPS_Span%s_BCZ_TL_%s_inst%s.txt'%(len(nodeArray),machineName,spanning,len(nodeArray),inst))    
            ...
            resultTable.to_csv(r'result%s/result_%s_N-1_PSPS_Span%s_BCZ_TL_%s.csv'%(len(nodeArray),machineName,spanning,len(nodeArray)), index = False)#Check
