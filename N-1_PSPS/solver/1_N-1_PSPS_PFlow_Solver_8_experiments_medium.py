...
for (numNodes,folderName) in [(30,'matpower/case%s'%(30)),(118,'matpower/case%s'%(118)),(300,'matpower/case%s'%(300))]:
    ...
    for inst in range(1,50+1):        
        ...
        model.setParam('LogFile', 'result%s/grblog_%s_Relax%s_N-1_PSPS_Span%s_PFlow_Lazy%s_%s_inst%s.txt'%(len(nodeArray),machineName,relax,spanning,laziness,len(nodeArray),inst))   
        ...
        optSolution.to_csv(r'result%s/opt_N-1_PSPS_Span%s_PFlow_Relax%s_%s_inst%s.csv'%(len(nodeArray),spanning,relax,len(nodeArray),inst), index = False)#Check
        ...
        if relax == False:
            ...
            resultTable.to_csv(r'result%s/result_%s_N-1_PSPS_Span%s_PFlow_Lazy%s_%s.csv'%(len(nodeArray),machineName,spanning,laziness,len(nodeArray)), index = False)#Check
