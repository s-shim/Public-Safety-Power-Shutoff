...
model.setParam('LogFile', 'result%s/grblog_%s_N-1_PSPS_Span%s_FlowBCMIPNODE_%s_inst%s.txt'%(len(nodeArray),machineName,spanning,len(nodeArray),inst))    
...
optSolution.to_csv(r'result%s/opt_N-1_PSPS_Span%s_FlowBCMIPNODE_%s_inst%s.csv'%(len(nodeArray),spanning,len(nodeArray),inst), index = False)#Check
...
resultTable.to_csv(r'result%s/result_%s_N-1_PSPS_Span%s_FlowBCMIPNODE_%s.csv'%(len(nodeArray),machineName,spanning,len(nodeArray)), index = False)#Check
