...
model.setParam('LogFile', 'result%s/grblogLCC2_%s_Relax%s_N-1_PSPS_Span%s_PFlow_Lazy%s_%s_inst%s.txt'%(len(nodeArray),machineName,relax,spanning,laziness,len(nodeArray),inst))   
...
model.setParam('GomoryPasses',100) # inst1    
...
optSolution.to_csv(r'result%s/optLCC2_N-1_PSPS_Span%s_PFlow_Relax%s_%s_inst%s.csv'%(len(nodeArray),spanning,relax,len(nodeArray),inst), index = False)#Check
...
if relax == False:
    ...
    resultTable.to_csv(r'result%s/resultLCC2_%s_N-1_PSPS_Span%s_PFlow_inst%s_Lazy%s_%s.csv'%(len(nodeArray),machineName,spanning,inst,laziness,len(nodeArray)), index = False)#Check
