...
model.setParam('LogFile', 'result%s/grblog/grblog_%s_N-1_PSPS_Span%s_BCZ_TL_Inc_%s_inst%s.txt'%(len(nodeArray),machineName,spanning,len(nodeArray),inst))    
...
model.setParam('RelaxLiftCuts',2) # BQPCuts 
model.setParam('BQPCuts',2) # BQPCuts 
model.setParam('PSDCuts',2)
...
def security(model, where):
    ...
    if where == GRB.callback.MIPNODE:
        ...
        if status == GRB.OPTIMAL:
            ...
            xVal = {}
            zVal = {}
            addDegree = {} ## for user defined incumbent later
            is_subNode_zero = {} ## for user defined incumbent later
            rxVal = {}
            for i in nodeArray:
                xVal[i] = model.cbGetNodeRel(X[i])
                rxVal[i] = xVal[i]
                # rxVal[i] = xVal[i] * random.random()
                addDegree[i] = 0
                is_subNode_zero[i] = 0
            rxVal[1] = 2
            ...
            for l in lineArray:
                simpleG[sourceNode[l]][targetNode[l]]['capacity'] = 0
                solFunction_zero[l] = 0

            relaxTotalRisk = 0
            for l in lineArray:
                zVal[l] = model.cbGetNodeRel(Z[l])
                relaxTotalRisk += riskFunction[l] * zVal[l]
                simpleG[sourceNode[l]][targetNode[l]]['capacity'] += zVal[l]
            ...
            for c in theContingent:
                ...
                if cut_valueC < xVal[sourceNode[c]] + xVal[targetNode[c]] - 1 - 0.0001:
            ...
            if violation < 0.0001:
                ...
                coparallel = 0
                for l in lineArray:
                    if - 2 * zVal[l] - 1 + xVal[sourceNode[l]] + xVal[targetNode[l]] > 0 + 0.0001:
                        for c in theContingent:
                            if c != l:
                                simpleGC = copy.deepcopy(simpleG)
                                simpleGC[sourceNode[c]][targetNode[c]]['capacity'] = simpleGC[sourceNode[c]][targetNode[c]]['capacity'] - zVal[c]
                                ...
                                reachableC, non_reachableC = partitionC
                                if cut_valueC < - 2 * zVal[l] - 1 + xVal[sourceNode[l]] + xVal[targetNode[l]] - 0.0001:
                                    ...
                                    model.cbCut(LinExpr(LHSC)>=-1)
                                    coparallel += 1
                print('coparallel =',coparallel)


            if model._bestTotalRisk - relaxTotalRisk > model._bestTotalRisk * 0.2: 
                # feed user defined incumbent
                sortedNodeArray = sorted(nodeArray, key = lambda i: rxVal[i], reverse = True)
                totalDBound = 0
                totalGenBound = 0
                theSubNodes = []
                is_subNode = copy.deepcopy(is_subNode_zero)
                bestSolRisk = totalRisk
                improve = 0
                for i in sortedNodeArray:
                    is_subNode[i] = 1
                    theSubNodes += [i]
                    ...
                    if totalDBound >= demandRequired and totalGenBound >= demandRequired:
                        ...
                        if nx.is_connected(subG) == True:
                            ...
                            if is_feasible == True:
                                solFunction = copy.deepcopy(solFunction_zero)
                ...
                if improve > 0 and model._bestTotalRisk > bestSolRisk:

                    print('grand total risk = %s > %s = incumbent total risk'%(model._bestTotalRisk , bestSolRisk))
                    model._bestTotalRisk = bestSolRisk
                   
                    print('### feed incumbent')
                                    
                    for l in lineArray:
                        model.cbSetSolution(Z[l],solFunction[l])
                        
                    for i in nodeArray:
                        model.cbSetSolution(X[i],is_subNode[i])            
...
print('### minimum possible risk =',bestValue)

# read the optimal solution
variableName = ['obj']
variableValue = [bestValue]
resultRisk = 0.0
resultDemand = 0.0
zValue = {}
...
minRiskArray += [bestValue]
minRatioRiskArray += [bestValue/totalRisk]


resultTable = pd.DataFrame(list(zip(sizeArray,instArray, totalDemandArray, ratioDemandRequiredArray, totalRiskArray, minRiskArray, resultRiskArray, minRatioRiskArray, ratioRiskArray, timeArray, nodecountArray, securityArray, machineArray, spanningArray)),columns =['Nodes', 'inst', 'totalDemand','ratioDemandRequired', 'totalRiskArray', 'minRisk', 'riskBd', 'ratioMinRisk', 'ratioBd', 'Time', 'B&B Nodes', 'Security', 'Machine', 'Spanning'])
resultTable.to_csv(r'result%s/result/result_%s_N-1_PSPS_Span%s_BCZ_Enhanced_%s_inst%s.csv'%(len(nodeArray),machineName,spanning,len(nodeArray),inst), index = False)#Check
